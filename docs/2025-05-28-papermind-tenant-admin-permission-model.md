...
`tenant_admin` 创建 `teacher` 用户时，只创建租户用户和租户级 `teacher` 角色，不自动加入任何空间。教师加入空间必须通过空间成员接口显式分配。
...
审计主体类型和 `user_roles` 单角色唯一约束都通过 `002_audit_actor_type.sql` 追加迁移落地，`001_tenant_space.sql` 保持已执行环境的历史基线。
