...
- 后端：Go + Gin
- 前端：React + Vite + TypeScript + Ant Design（作为底层组件能力，现有管理端和考试端视觉结构保持不变）
- ORM：GORM
...
├── invite_code           # 考试邀请码
├── status                # 考试状态：draft / published / closed / disabled
├── created_by            # 创建人用户 ID
├── created_at            # 创建时间
├── updated_at            # 更新时间
├── updated_by            # 更新人用户 ID
...
`invite_code` 必须全局唯一。公开入口只按邀请码解析已发布考试，数据库唯一约束和服务端生成碰撞检查都必须避免两个租户生成相同邀请码。考试可先以 `draft` 保存发布目标和时间配置，草稿态允许在原考试范围授权内继续调整试卷、时间、作答规则、成绩发布规则和投放目标，也允许有权限的调用方软删除；后续通过状态更新发布为 `published` 时再生成邀请码并冻结 `rule_live` 题池。空间角色执行发布、提前结束、禁用或删除草稿时，后端必须确认考试所有 `exam_target_scope_spaces` 均落在当前账号启用授权空间内；教师只能操作自己创建的考试，历史 `created_by = 0` 数据只在 `draft` 状态按授权空间兼容处理，发布后不能继续作为教师可操作依据。`closed` 表示提前结束考试并把 `end_time` 写为操作时间，`disabled` 表示禁用考试，非 `published` 状态不得解析邀请码或开始作答。已发布、已结束和已禁用考试不得删除，避免破坏作答、成绩和审计链路。
...
同一套试卷可以被多次发布为不同考试；同一场考试也支持同一学生多次作答。`exams.max_attempts` 控制最多作答次数，数据库默认值为 1；发布接口传 0 表示不限制作答次数。`exams.result_strategy` 控制多次作答的成绩采用方式，首版支持：

- `latest`：取最后一次提交成绩。
- `highest`：取最高分。

首版为了保持阅卷和成绩发布流程清晰，包含简答题的试卷必须设置 `max_attempts = 1`。多次作答和不限制作答只支持纯客观题试卷。这样 `result_strategy` 只需要在自动判分结果之间计算，不进入多次人工阅卷流程。
...
4. 如果没有进行中的 attempt，统计已创建 attempt 数量；当 `max_attempts > 0` 时不能超过该次数上限。
...
- 前端新建考试使用右侧抽屉承载表单，必须由用户显式选择创建状态：`draft` 只保存考试配置和发布目标，不生成邀请码；`published` 立即发布并生成邀请码。草稿考试在列表中提供编辑和删除入口，编辑复用新建抽屉保存草稿配置，删除前必须二次确认；考试列表同时提供详情入口跳转 `/exams/:id`。列表操作按钮按考试状态展示，当前登录人没有操作权限时按钮置灰禁用。表单提供两种录入模式：固定场次考试选择同一天的开始/结束时间，`duration_minutes` 由时间差自动计算；开放时间窗考试使用 Ant Design 双月日期范围选择开放日期，前端按起始日 `00:00` 到结束日 `23:59` 写入统一时间窗口，并显式填写单次作答时长。指定同学发布范围使用本地组合框按学生姓名筛选后加入已选名单。考试详情的考生管理列表不提供批量导入考生入口，右侧筛选区按搜索、考试状态、班级三项同排展示；发送或重发邀请码调用 `/api/v1/exams/:id/invitations/resend`，由后端过滤真实应考名单和授权空间后返回当前考试邀请码并写入审计日志。教师角色的新建考试发布范围固定为当前进入空间；指定人群只通过 `/api/v1/spaces/:id/members?role=student&status=enabled` 筛选同空间启用学生；教师可只读引用启用公共试卷发布到授权空间，并可对自己创建的当前授权空间考试执行草稿保存、草稿删除、发布、提前结束和禁用，但后端不得因此开放教师的空间成员管理、公共试卷维护或教师/禁用成员读取权限。
...
操作区。新建试卷必须先填写试卷名称、组卷方式、考试时长和适用年级，基础必填项以红色星号标记；试卷说明使用 Markdown 编辑器维护并按原文保存；保存基础信息后再进入组卷页；已创建试卷的组卷方式在前端只读展示，不允许在编辑或规则管理过程中二次切换。工作台至少包含三块：试卷列表与模式展示、大题与已选题工作区、规则工作区。智能组卷的题型数量与分值区必须复用真实大题结构，支持新增题型、删除题型和拖拽排序；左侧题型序号与右侧已生成试卷分组都按当前大题顺序即时重算。`manual`
...
- 租户后台“空间成员”页面必须直接调用 `/api/v1/tenant/spaces/:id/members` 这一组接口完成成员列表、添加成员、修改空间身份、启用/禁用和移除；目标空间来自当前 session 的授权空间列表，不允许使用前端固定租户或空间默认值。租户管理员不展示独立“空间成员”菜单，避免和“空间管理”里的成员管理入口重复；租户管理员从“空间管理 > 成员管理”打开右侧抽屉查看空间成员，抽屉复用租户资源抽屉的遮罩、滑入、全屏和关闭动画，抽屉主体提供添加成员入口、成员检索输入框和搜索/刷新图标；成员检索、分页和刷新只作用于当前打开空间的成员集合。成员抽屉列表不展示操作列，不提供禁用、启用、降级或详情入口；新增成员可从当前成员抽屉预选空间后提交，空间管理员调整从空间管理页编辑空间入口完成。独立 `/space-members` 入口只给当前账号具备启用 `space_admin` 空间授权的用户。
...
GET  /api/v1/questions/tags
     query: tenant_id, space_id?, scope?, status?, search?；返回当前范围内可见题目的标签候选，过滤和去重在后端完成。
POST /api/v1/questions/availability-counts
     body: tenant_id, space_id?, scope?, status?, tags[], exclude_question_ids[]；返回当前范围内按题型聚合的可用题量，供智能组卷在后端完成过滤和计数。
...
GET  /api/v1/papers/:id/questions
     query: tenant_id；返回当前试卷已固化题目列表，包含题型、标题、难度、标签名和选项等展示元数据，供手动组卷和 rule_fixed 审题共用。
...
GET  /api/v1/exams
     ...
     response: items[] 包含 paper_name、created_by、targets[]；前端列表直接使用 paper_name 展示试卷列，草稿邀请码显示“待生成”，范围列按目标类型展示“指定空间”或“指定考生”。
POST /api/v1/exams
...
DELETE /api/v1/exams/:id
     query: tenant_id；仅允许删除 draft 考试，成功后软删除 `exams.deleted_at`，默认列表和详情不可见；已发布、已结束或已禁用考试返回业务错误。教师只能删除自己创建且完全落在当前启用授权空间内的草稿考试，`created_by = 0` 的历史草稿只按授权空间兼容删除。
...
POST /api/v1/exams/:id/invitations/resend
     body: tenant_id, space_id?, user_ids[]；校验当前授权范围内的真实应考名单，返回当前考试邀请码并记录 send_invite 审计日志；当前版本没有短信或邮件发送通道。
...
列表页的搜索、状态、题型、难度、标签、空间范围等过滤必须在后端查询条件中完成，
并且先过滤再统计 `total` 和分页；前端只负责传递条件、展示当前页和触发翻页，
不得在浏览器端对全量结果做业务过滤或分页。
...
- 空间支持禁用。租户管理员禁用空间前必须确认题库、试卷、考试、阅卷和成绩等关联业务停止操作的风险；禁用后空间仍保留在空间管理列表中展示禁用状态，空间下业务读写必须按启用空间过滤。
...
- 同一考生的一次考试中，同一道题不允许重复出现。
- 同一场考试支持同一学生多次作答，由 `max_attempts` 控制次数，`max_attempts = 0` 表示不限制作答次数，由 `result_strategy` 控制成绩采用方式。
- 含简答题试卷首版不允许多次作答或不限制作答，`max_attempts` 必须为 1。
- 支持服务端自动保存答案。
