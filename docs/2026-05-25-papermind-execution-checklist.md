...
- [x] 新建用户表单改为右侧抽屉展示，姓名、账号、初始密码和角色必填项显示红色星号。
- [x] 用户管理操作区支持编辑教师或学生的空间归属，用户角色在编辑抽屉中只读不可变更。
- [x] 用户导入。
- [x] 用户头像上传。
- [x] 用户禁用提示。
...
- [x] 空间管理列表中的“成员管理”改为复用右侧资源抽屉展示，抽屉内支持添加成员、当前空间内检索、分页和刷新；成员抽屉不再展示操作列，成员新增可从当前成员抽屉预选空间后提交，空间管理员调整从空间管理页既有入口完成。
- [x] 租户管理列表中的空间和用户入口改为右侧抽屉查看，独立于租户侧空间管理和用户管理页面，并保持原空间、用户列表列结构。
- [x] 租户管理资源抽屉默认占用 50% 视口宽度，支持全屏展开和退出全屏，宽度切换保持动画。
- [x] 租户管理资源抽屉使用 Ant Design Drawer 原生遮罩、展开动画和收起动画，不在父级布局中预留白色占位区域。
- [x] 租户管理资源抽屉点击遮罩层、返回按钮或关闭按钮都会收起抽屉。
- [x] 租户管理资源抽屉初始位于视口外，按 Ant Design Drawer 原生动效滑入，避免先闪现白色面板。
...
- [x] 管理员可以创建租户、空间、用户。
- [x] 教师可以完成出题、组卷、发布考试。
- [x] 教师发布考试时发布范围固定为当前空间，指定人群仅筛选当前空间启用学生。
- [x] 教师可以只读引用公共试卷发布到当前授权空间，公共试卷内容维护权限仍仅限租户管理员。
- [x] 教师可以编辑当前授权空间内草稿考试，并可发布、提前结束、禁用当前授权空间内考试，考试列表提供详情入口。
- [x] 考生可以进入考试、答题、自动保存、提交。
- [x] 教师可以阅卷。
- [ ] 授权目标空间内修改成绩发布配置，并由后端按阅卷完成状态和公布时间统一校验学生可见性。
- [x] 成绩导出可用。
...
- [ ] 多选答案排序和反序列化判分已覆盖。
- [x] 防作弊事件异步写入已覆盖。
- [x] 阅卷、基础成绩发布配置、导出主链路已覆盖。
- [ ] 授权目标空间内修改成绩发布配置和学生可见性统一校验仍待补测。
- [x] React 页面闭环已覆盖。
- [ ] 前后端 API 接入已覆盖。
- [ ] 部署、压测、文档收口已覆盖。
