...
test("租户学生登录成功后保存登录态并进入考试入口", async () => {
	const user = userEvent.setup();
	const api: AuthAPI = {
		platformLogin: vi.fn(),
    tenantLogin: async (input) => {
      expect(input).toEqual({ tenantID: 10, username: "student01", password: "papermind123" });
      return {
  ...
  render(
    <SessionProvider>
      <MemoryRouter initialEntries={["/login"]}>
        <Routes>
          <Route path="/login" element={<PlatformLoginPage api={api} />} />
          <Route path="/exam-entry" element={<div>考试入口页面</div>} />
        </Routes>
      </MemoryRouter>
    </SessionProvider>,
  );
...
test("租户登录成功后拉取授权空间并保存到 session", async () => {
	...
	expect(screen.getByText("考试平台概览")).toBeInTheDocument();
	expect(listProfileSpaces).toHaveBeenCalledTimes(1);
	const storedSession = JSON.parse(window.localStorage.getItem(SESSION_STORAGE_KEY) ?? "{}");
	expect(storedSession.profileSpaces).toEqual([{
		id: 1,
		tenantID: 10,
		spaceID: 301,
		role: "space_admin",
