...
```powershell
.\deployments\sqlite-single-node\start.ps1 -Port 8080 -DatabaseFile "server/data/sqlite/papermind.db"
```
...
脚本注入的 SQLite DSN 固定包含首版必需参数：

```text
file:server/data/sqlite/papermind.db?_foreign_keys=on&_journal_mode=WAL&_busy_timeout=5000
```
...
- 文件目录使用 `server/data/tmp`、`server/data/imports`、`server/data/exports`
