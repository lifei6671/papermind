...
* 查询数据库尽量使用简单查询，不能一次性查出所有数据，非必要不能 join 表或用复杂数据库函数。
* SQLite 迁移必须保留 `_foreign_keys=on` 可验证的外键语义。
* `ext_json` 只允许保存展示类、非核心扩展元数据。参与计算、排序、过滤、权限判断、状态流转、发布校验、任务分配、唯一性、幂等、审计聚合、去重或分页的字段，必须使用显式列、关系表或可索引的结构化字段，不得放入 `ext_json`。
* 有独立业务语义的 JSON 字段不等同于 `ext_json`，但不得依赖 JSON key 做数据库过滤、排序、权限裁剪、唯一性或分页；如果 JSON 内字段进入核心流程，必须迁移为显式列、独立子表或同步冗余字段。
* 可以为了减少 join 冗余字段，但冗余字段必须有清晰来源、事务内同步策略、索引或唯一约束设计，以及覆盖该冗余字段的测试。
* Review 数据库设计、迁移脚本、DO 结构体和 repository 写入路径时，必须主动检查是否把核心业务字段藏进了 `ext_json`。
* 新增字段时，同步检查：
